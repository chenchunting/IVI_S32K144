...
# 57 "D:/NXP/SourceCode/s32k144/s32k144/Generated_Code/can_pal1.h" 2
...
# 7 "../Sources/can_receiver.c" 2
# 1 "D:/NXP/SourceCode/s32k144/s32k144/Generated_Code/lpuart1_uart.h" 1
# 8 "../Sources/can_receiver.c" 2
...
void can_receive(void)
{
 _Bool repeat_receive = 0;
 uint32_t bytesRemaining;
 uint8_t i=0,j=0,recvMsg_length=0,checksum=0,checksum_buff=0;
 uint8_t uart_reply_recvMsg[20]={0};
 uint8_t uart_tranMsg[20]={0};
# 80 "../Sources/can_receiver.c"
 WDG_Refresh(WDG_OVER_WDOG_INSTANCE);
 while(repeat_receive == 0)
 {
  CAN_Receive((0U), (0UL), &can_recvMsg_1);
  WDG_Refresh(WDG_OVER_WDOG_INSTANCE);
  while(CAN_GetTransferStatus((0U), (0UL)) == STATUS_BUSY);

  if(can_recvMsg_1.id == 0x602)
  {
   if(can_recvMsg_1.data[0]==0xAA || can_recvMsg_1.data[0]==0xA5)
   {
    ...
    if(checksum == checksum_buff)
    {
     ...
     if(can_recvMsg_1.data[0]==0xAA)
     {
      if(j != 0)
      {
       ...
       CAN_Send((0U), (1UL), &can_tranvMsg_1);
       WDG_Refresh(WDG_OVER_WDOG_INSTANCE);

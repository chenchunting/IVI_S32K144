...
void dms_test(void)
{
 ...
 for(i=0;i<len;i++)
 {
  UART_ReceiveData((0U), &uart_recvMsg[i], 1UL);
  while(UART_GetReceiveStatus((0U), &bytesRemaining) != STATUS_SUCCESS);
 }

 can_tranvMsg_0.length=len;
 can_tranvMsg_0.id=0x601;
 for(i=0;i<len;i++) can_tranvMsg_0.data[i]=uart_recvMsg[i];
 CAN_Send((0U), (1UL), &can_tranvMsg_0);
 WDG_Refresh(WDG_OVER_WDOG_INSTANCE);
 while(CAN_GetTransferStatus((0U), (1UL)) == STATUS_BUSY);

 CAN_Receive((0U), (0UL), &can_recvMsg_2);
 WDG_Refresh(WDG_OVER_WDOG_INSTANCE);
 while(CAN_GetTransferStatus((0U), (0UL)) == STATUS_BUSY);

 UART_SendData((0U), can_recvMsg_2.data, can_recvMsg_2.length);
 WDG_Refresh(WDG_OVER_WDOG_INSTANCE);
...
void uart_receive(void)
{
 ...
 while(received == 0)
 {
  ...
  if(i>=20)
  {
   ...
   UART_SendData((0U), uart_error_length, len_uart_error_length);
   WDG_Refresh(WDG_OVER_WDOG_INSTANCE);
   while(UART_GetTransmitStatus((0U), &bytesRemaining) != STATUS_SUCCESS);
  }
 }
...
# 158 "../Sources/uart_receiver.c"
     ...
     else
     {
      error_code = 1;
      repeat_transmit = 1;
      UART_SendData((0U), uart_error_checksum, len_uart_error_checksum);
      WDG_Refresh(WDG_OVER_WDOG_INSTANCE);
      while(UART_GetTransmitStatus((0U), &bytesRemaining) != STATUS_SUCCESS);
     }
...
# 208 "../Sources/uart_receiver.c"
   ...
   if(can_recvMsg_0.data[0] == 0xAA && can_recvMsg_0.data[1] == 0x04 && can_recvMsg_0.data[2] == 0xCC && can_recvMsg_0.data[3] == 0x84)
   {
    ...
    UART_SendData((0U), uart_success, len_uart_success);

   }
   else if(can_recvMsg_0.data[0] == 0xAA && can_recvMsg_0.data[1] == 0x04 && can_recvMsg_0.data[2] == 0xE0 && can_recvMsg_0.data[3] == 0x70) UART_SendData((0U), uart_error_header, len_uart_error_header);
   else if(can_recvMsg_0.data[0] == 0xAA && can_recvMsg_0.data[1] == 0x04 && can_recvMsg_0.data[2] == 0xE1 && can_recvMsg_0.data[3] == 0x6F) UART_SendData((0U), uart_error_length, len_uart_error_length);
   else UART_SendData((0U), uart_error_checksum, len_uart_error_checksum);
   WDG_Refresh(WDG_OVER_WDOG_INSTANCE);
   while(UART_GetTransmitStatus((0U), &bytesRemaining) != STATUS_SUCCESS);

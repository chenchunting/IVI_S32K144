...
#define TX_MSG_ID 0x601
#define RX_MAILBOX (0UL)
#define RX_MSG_ID 0x602
...
# 24 "C:/NXP/S32DS_ARM_v2018.R1/S32DS/S32SDK_S32K14x_EAR_0.8.6/platform/drivers/inc/flexcan_driver.h" 2
...
#define Send_H 
# 76 "D:/NXP/SourceCode/s32k144/s32k144/Generated_Code/Send.h"
#define SEND (0U)
...
# 56 "D:/NXP/SourceCode/s32k144/s32k144/Generated_Code/lpuart1_uart.h" 2
...
void uart(void)
{
 ...
 _Bool received = 0;
# 28 "../Sources/test.c"
 UART_SendData((0U), uart_test_number1, 4);
 while(UART_GetTransmitStatus((0U), &bytesRemaining) != STATUS_SUCCESS);
 OSIF_TimeDelay(500);
}
...
void hx8880_spi(void)
{
 ...
 LPSPI_DRV_MasterTransfer((1U), m_send,m_receive, 8U);
 OSIF_TimeDelay(20);
 LPSPI_DRV_SlaveTransfer((0U), s_send,s_receive, 8U);
 OSIF_TimeDelay(20);

 for(i=0;i<8;i++)uart_test_number1[i]=s_receive[i];
 UART_SendData((0U), uart_test_number1, 8);
 while(UART_GetTransmitStatus((0U), &bytesRemaining) != STATUS_SUCCESS);
...
void pwm_test(void)
{
 FTM_DRV_UpdatePwmChannel(0U, 1U, FTM_PWM_UPDATE_IN_DUTY_CYCLE, 0x6000, 0U, 1);
 OSIF_TimeDelay(500);
 FTM_DRV_UpdatePwmChannel(0U, 1U, FTM_PWM_UPDATE_IN_DUTY_CYCLE, 0x2000, 0U, 1);
 OSIF_TimeDelay(500);
}

void write_pin(void)
{
 PINS_DRV_WritePin(((GPIO_Type *)(0x400FF0C0u)), 15U, 0u);
 PINS_DRV_SetPins(((GPIO_Type *)(0x400FF0C0u)), (1 << 15U) | (1 << 16U));
}
...
{
 _Bool received = 0,repeat_transmit = 0;
 uint32_t bytesRemaining;
 uint8_t i=0,j=0,recvMsg_length=0,length_buff=0,checksum=0,checksum_buff=0;
 uint8_t uart_recvMsg[20]={0},uart_tranMsg[20]={0},uart_reply_recvMsg[20]={0};;
 ...
 while(received == 0)
 {
  UART_ReceiveData((1U), &uart_recvMsg[i], 1UL);
  while(UART_GetReceiveStatus((1U), &bytesRemaining) != STATUS_SUCCESS);
  i++;
  if(i>=1)if(i == uart_recvMsg[1]) received = 1;

  if(i>=20)
  {

   UART_SendData((1U), uart_error_length, len_uart_error_length);
   while(UART_GetTransmitStatus((1U), &bytesRemaining) != STATUS_SUCCESS);
  }
 }

 if(uart_recvMsg[0]==0xAA)
 {
  ...
  if(checksum == checksum_buff)
  {
   if(recvMsg_length<=8)
   {
    ...
    CAN_Send((0U), (1UL), &can_tranvMsg_0);
    while(CAN_GetTransferStatus((0U), (1UL)) == STATUS_BUSY);
   ...
   else
   {
    ...
    while(repeat_transmit == 0)
    {
     ...
     else
     {
      ...
      CAN_Send((0U), (1UL), &can_tranvMsg_2);
      while(CAN_GetTransferStatus((0U), (1UL)) == STATUS_BUSY);
     }
   ...
   UART_SendData((0U), uart_tranMsg, uart_tranMsg[1]);
   while(UART_GetTransmitStatus((0U), &bytesRemaining) != STATUS_SUCCESS);


   for(i=0;i<4;i++)
   {
    UART_ReceiveData((0U), &uart_reply_recvMsg[i], 1UL);
    while(UART_GetReceiveStatus((0U), &bytesRemaining) != STATUS_SUCCESS);
   }
   ...
   if(can_tranvMsg_2.data[0] == 0xAA)
   {
    if(can_tranvMsg_2.data[1] == 0x04)
    {
     ...
     {
      UART_SendData((1U), uart_success, len_uart_success);

     }
     else if(can_tranvMsg_2.data[2] == 0xE0 && can_tranvMsg_2.data[3] == 0x70) UART_SendData((1U), uart_error_header, len_uart_error_header);
     else if(can_tranvMsg_2.data[2] == 0xE1 && can_tranvMsg_2.data[3] == 0x6F) UART_SendData((1U), uart_error_length, len_uart_error_length);
     else UART_SendData((1U), uart_error_checksum, len_uart_error_checksum);
     while(UART_GetTransmitStatus((1U), &bytesRemaining) != STATUS_SUCCESS);
    }
   }
  }
  else
  {
   ...
   UART_SendData((1U), uart_error_checksum, len_uart_error_checksum);
   while(UART_GetTransmitStatus((0U), &bytesRemaining) != STATUS_SUCCESS);
  }
 }
 ...
 else
 {
  ...
  UART_SendData((1U), uart_error_header, len_uart_error_header);
  while(UART_GetTransmitStatus((0U), &bytesRemaining) != STATUS_SUCCESS);
 }
